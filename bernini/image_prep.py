...
def fit_long_edge(image: torch.Tensor, max_edge: int, stride: int = 16) -> torch.Tensor:
    """Scale *image* so its longer side is at most *max_edge*, preserving aspect ratio."""
    rgb = image[..., :3]
    ...
    new_h = snap_dimension(int(round(height * scale)), stride)
    new_w = snap_dimension(int(round(width * scale)), stride)
