...
class WrappedPreviewer(LatentPreviewer):
    ...
    def decode_latent_to_preview_image(self, preview_format, x0):
        ...
        if self.first_preview:
            ...
            _prompt_server().send_sync('VHS_latentpreview', {'length':num_images, 'rate': self.rate, 'id': _prompt_server().last_node_id})
    ...
    def process_previews(self, image_tensor, ind, leng):
        ...
        for preview in previews_ubyte:
            ...
            message.write(struct.pack('16p', _prompt_server().last_node_id.encode('ascii')))
            i.save(message, format="JPEG", quality=95, compress_level=1)
            import server
            _prompt_server().send_sync(server.BinaryEventTypes.PREVIEW_IMAGE,
                           message.getvalue(), _prompt_server().client_id)
            if self.rate == 16:
        ...
        if self.swarmui_env:
            ...
            _prompt_server().send_sync(1, preview_bytes, sid=_prompt_server().client_id)
    def decode_latent_to_preview(self, x0):
