...
try:
    ...
    if not hasattr(torch.ops.wananimateplus, 'sageattn'):
        @torch.library.custom_op("wananimateplus::sageattn", mutates_args=())
    ...
    sageattn_func = torch.ops.wananimateplus.sageattn
...
try:
    ...
    if not hasattr(torch.ops.wananimateplus, 'sageattn_varlen'):
        @torch.library.custom_op("wananimateplus::sageattn_varlen", mutates_args=())
        ...
        def _(q, k, v, q_lens, k_lens, max_seqlen_q, max_seqlen_k, dropout_p=0.0, is_causal=False):
            return q.clone()
    sageattn_varlen_func = torch.ops.wananimateplus.sageattn_varlen
except Exception:
...
try:
    ...
    if not hasattr(torch.ops.wananimateplus, 'sageattn_ultravico'):
        @torch.library.custom_op("wananimateplus::sageattn_ultravico", mutates_args=())
    ...
    sageattn_func_ultravico = torch.ops.wananimateplus.sageattn_ultravico
except Exception:
