...
from .utils import(log, print_memory, apply_lora, fourier_filter, optimized_scale, setup_radial_attention,
                   ...
                   flow_match_add_noise as add_noise)
...
class WanVideoSampler:
    ...
    def INPUT_TYPES(s):
        return {
            ...
            "optional": {
                ...
                "guidance_mode": (
                    BERNINI_GUIDANCE_MODES,
                    {
                        "default": "none",
                        "tooltip": (
                            "Bernini guidance: none = standard CFG; apg / *_apg = Adaptive Projected "
                            "Guidance; rv2v / v2v / v2v_chain / t2v = official modes (standard CFG in this wrapper)."
                        ),
                    },
                ),
    ...
    def process(self, model, image_embeds, shift, steps, cfg, seed, scheduler, riflex_freq_index, text_embeds=None,
        ...
        if gguf_reader is not None: #handle GGUF
            ...
            set_lora_params(transformer, patcher.patches, state_dict=model["sd"], compute_dtype=dtype)
        else:
        ...
        if multitalk_sampling:
            raise ValueError(
                "Multitalk is not supported in ComfyUI-Bernini. "
                "Use Bernini Context Embeds + Sampler for video editing workflows."
            )
        ...
        # HuMo (removed — not used in Bernini workflows)
        humo_audio = humo_audio_neg = humo_image_cond = humo_image_cond_neg = None
        ...
        from .bernini_unsupported import reject_pose_face_inputs
        reject_pose_face_inputs(image_embeds)
        ...
        # MTV Crafter (removed — not used in Bernini workflows)
        mtv_input = None
        ...
        if uni3c_embeds is not None:
            ...
            if render_latent.shape != noise.shape:
                render_latent = torch.nn.functional.interpolate(
                    render_latent, size=(noise.shape[1], noise.shape[2], noise.shape[3]),
        ...
        # WanMove (removed — not used in Bernini workflows)
        wanmove_embeds = None
        ...
        _momentum_buf = MomentumBuffer(apg_momentum) if uses_apg_guidance(guidance_mode) else None
        ...
        def predict_with_cfg(z, cfg_scale, positive_embeds, negative_embeds, timestep, idx, image_cond=None, clip_fea=None,
                             ...
                             uni3c_data=None, latent_model_input_ovi=None, flashvsr_LQ_latent=None,
            ...
            with torch.autocast(device_type=mm.get_autocast_device(device), dtype=dtype) if autocast_enabled else nullcontext():
                ...
                elif uses_apg_guidance(guidance_mode):
        ...
        for iter_idx in range(iterations):
            ...
            try:
                ...
                for idx, t in enumerate(tqdm(timesteps[ttm_start_step:], disable=multitalk_sampling)):
                    ...
                    #region normal inference
                        ...
                        noise_pred, noise_pred_ovi, self.cache_state = predict_with_cfg(
                            ...
                            humo_image_cond=humo_image_cond, humo_image_cond_neg=humo_image_cond_neg, humo_audio=humo_audio, humo_audio_neg=humo_audio_neg,
                            uni3c_data = uni3c_data, latent_model_input_ovi=latent_model_input_ovi, flashvsr_LQ_latent=flashvsr_LQ_latent,
                            context_latents=context_latents,
                        )
                        if bidirectional_sampling:
