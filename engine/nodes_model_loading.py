...
from comfy.utils import load_torch_file, ProgressBar, convert_old_quants
...
def _dequantize_scaled_fp8_state_dict(sd, dtype=torch.bfloat16):
    """Convert ComfyUI scaled_fp8 weights to compute dtype for eager module loading."""
    sd = dict(sd)
    if "scaled_fp8" not in sd and not any(
        k.endswith(".scale_weight") or k.endswith(".weight_scale") for k in sd
    ):
        return sd

    sd, _ = convert_old_quants(sd, "", metadata={})
    out = {}
    skip_suffixes = (".weight_scale", ".input_scale", ".comfy_quant")
    ...
    for k, v in sd.items():
        ...
        scale_key = f"{k[:-len('.weight')]}.weight_scale"
        ...
        weight = v.to(torch.float32) if v.dtype in (torch.float8_e4m3fn, torch.float8_e5m2) else v.to(dtype)
        sw = sd[scale_key].to(dtype)
        if sw.ndim > 1 and sw.shape[-1] != weight.shape[-1]:
            block_size = weight.shape[-1] // sw.shape[-1]
            if block_size > 1:
                sw = sw.repeat_interleave(block_size, dim=-1)
        out[k] = (weight.to(dtype) * sw).to(dtype)
...
class LoadWanVideoT5TextEncoder:
    ...
    def loadmodel(self, model_name, precision, load_device="offload_device", quantization="disabled"):
        ...
        is_scaled_fp8 = "scaled_fp8" in sd or any(
            k.endswith(".scale_weight") or k.endswith(".weight_scale") for k in sd
        )
        if is_scaled_fp8:
            log.info("Dequantizing scaled fp8 T5 weights to %s for loading", precision)
            sd = _dequantize_scaled_fp8_state_dict(sd, dtype)
